...
describe("ProviderRuntimeIngestion", () => {
  ...
  it("does not materialize an empty assistant message when completion has no text", async () => {
    const harness = await createHarness();
    const now = new Date().toISOString();
    ...
    harness.emit({
      type: "request.opened",
      eventId: asEventId("evt-request-opened-empty-assistant"),
      provider: "copilot",
      createdAt: now,
      threadId: asThreadId("thread-1"),
      turnId: asTurnId("turn-empty"),
      requestId: ApprovalRequestId.makeUnsafe("req-empty-assistant"),
      payload: {
        requestType: "dynamic_tool_call",
        detail: "github/searchIssues",
      },
    });
    ...
    const thread = await waitForThread(harness.engine, (entry) =>
      entry.activities.some(
        (activity: ProviderRuntimeTestActivity) =>
          activity.id === "evt-request-opened-empty-assistant",
      ),
    );
    ...
    const approval = thread.activities.find(
      (activity: ProviderRuntimeTestActivity) =>
        activity.id === "evt-request-opened-empty-assistant",
    );
    expect(approval?.kind).toBe("approval.requested");
  });
  ...
  it("maps dynamic tool approvals into generic tool approval activities", async () => {
    const harness = await createHarness();
    const now = new Date().toISOString();

    harness.emit({
      type: "request.opened",
      eventId: asEventId("evt-tool-request-opened"),
      provider: "copilot",
      createdAt: now,
      threadId: asThreadId("thread-1"),
      requestId: ApprovalRequestId.makeUnsafe("req-tool-open"),
      payload: {
        requestType: "dynamic_tool_call",
        detail: "github/searchIssues",
      },
    ...
    harness.emit({
      type: "request.resolved",
      eventId: asEventId("evt-tool-request-resolved"),
      provider: "copilot",
      createdAt: now,
      threadId: asThreadId("thread-1"),
      requestId: ApprovalRequestId.makeUnsafe("req-tool-open"),
    ...
    await waitForThread(
      ...
      (entry) =>
        entry.activities.some(
          (activity: ProviderRuntimeTestActivity) => activity.id === "evt-tool-request-opened",
        ) &&
        entry.activities.some(
          (activity: ProviderRuntimeTestActivity) => activity.id === "evt-tool-request-resolved",
        ),
    );
    ...
    const resolvedPayload =
      resolved?.payload && typeof resolved.payload === "object"
        ? (resolved.payload as Record<string, unknown>)
        : undefined;
    expect(resolvedPayload?.requestKind).toBe("tool");
    expect(resolvedPayload?.requestType).toBe("dynamic_tool_call");
  });
