...
const make = Effect.gen(function* () {
  ...
  const hasBufferedAssistantText = (messageId: MessageId) =>
    Cache.getOption(bufferedAssistantTextByMessageId, messageId).pipe(
  ...
  const processRuntimeEvent = (event: ProviderRuntimeEvent) =>
    Effect.gen(function* () {
      ...
      if (assistantCompletion) {
        ...
        const bufferedAssistantTextExists = yield* hasBufferedAssistantText(assistantMessageId);
        const hasFallbackCompletionText =
          (assistantCompletion.fallbackText?.trim().length ?? 0) > 0;
        const shouldFinalizeAssistantMessage =
          existingAssistantMessage !== undefined ||
          bufferedAssistantTextExists ||
          hasFallbackCompletionText;
        const shouldApplyFallbackCompletionText =
          !existingAssistantMessage || existingAssistantMessage.text.length === 0;
        if (!shouldFinalizeAssistantMessage) {
          yield* clearAssistantMessageState(assistantMessageId);
          if (turnId) {
