...
const autoApprovePermissionRequest: NonNullable<SessionConfig["onPermissionRequest"]> = (...args) =>
  loadCopilotSdk().then(({ approveAll }) => approveAll(...args));
...
const makeCopilotAdapter = (_options?: CopilotAdapterLiveOptions) =>
  Effect.gen(function* () {
    ...
    const makeClient = async (input: ProviderSessionStartInput): Promise<CopilotClient> => {
      const copilotOptions = input.providerOptions?.copilot;
