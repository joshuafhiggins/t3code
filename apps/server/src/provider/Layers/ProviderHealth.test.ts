...
it.layer(NodeServices.layer)("ProviderHealth", (it) => {
					...
					Effect.provide(
						...
						mockSpawnerLayer((args) => {
							...
							if (joined === "--version")
								return { stdout: "codex 1.0.0\n", stderr: "", code: 0 };
							throw new Error(
								`Auth probe should have been skipped but got args: ${joined}`,
							);
						}),
					),
				),
	...
	describe("checkCopilotProviderStatusWith", () => {
		...
		it.effect(
			"returns unauthenticated when copilot auth probe reports logged out",
			() =>
				Effect.gen(function* () {
					const status = yield* checkCopilotProviderStatusWith(() => ({
		...
		it.effect("returns unavailable when the copilot sdk cannot be loaded", () =>
			Effect.gen(function* () {
				const status = yield* checkCopilotProviderStatusWith(async () => {
					throw new Error(
						"Failed to load GitHub Copilot SDK: Cannot find module 'vscode-jsonrpc/node'",
					);
				});
				...
				assert.strictEqual(
					status.message,
					"GitHub Copilot SDK is unavailable in the current runtime. Failed to load GitHub Copilot SDK: Cannot find module 'vscode-jsonrpc/node'",
				);
			}),
		);
	});
	...
	describe("readCodexConfigModelProvider", () => {
		...
		it.effect(
			"returns the provider when model_provider is set at top level",
			() =>
				Effect.gen(function* () {
					yield* withTempCodexHome(
						'model = "gpt-5-codex"\nmodel_provider = "portkey"\n',
